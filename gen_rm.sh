...
#Assets路径 各文件夹的根目录
folderoot=$basepath/Assets/Samples
...
findpath="..\/..\/..\/SCREENRECORDS"
replacepath=SCREENRECORDS
...
for(( i=$filecnt-1;i>=0;i--)) do
	...
	while read line
	do
		# 替换掉 "../../../SCREENRECORDS" 这样的路径 并将内容追加到主README.md 
		echo $line | sed -e 's/'"$findpath"'/'"$replacepath"'/g' >> $mainfile
	done < $filename
	echo '***' >>$mainfile
done
